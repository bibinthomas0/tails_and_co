...
def CartAdd(request,id):
    ...
    if request.session['item_added_to_cart'] == True:
        key='2'
        cartdetail_url = reverse('cartdetail')
        product_link = f'<a href="{cartdetail_url}">Go to cart</a>'
        messages.error(request, f'Item added to Cart..{product_link} ({key})')
...
def CartDetail(request):
    ...
    cart_items = cart_items.annotate(total_product_price=F('product__price') * F('quantity'))
    total_price = cart_items.aggregate(Sum('total_product_price'))['total_product_price__sum']
    ...
    context={'cart_items': cart_items, 'total_price': total_price, 
             'total_stock': total_stock,'numitems':numitems,'cart':cart,'subtotal':subtotal,}
...
def Checkout(request):
    ...
    cart_items = CartItem.objects.filter(cart=cart)
    k=15
    total_price = int(sum((item.product.price * item.quantity for item in cart_items)))+k
    ...
    print(payment)
    context={'address':address,'cart_items':cart_items,'total_price':total_price,'numitems':numitems,
            'dis':dis,'subtotal':subtotal,'payment':payment}
...
def create_order(request):
    ...
    try:
        ...
        order = Order.objects.create(user=request.user,address=address,total_price=total_price,payment_method=payment1,coupon_applied=coup)
    except:
        total_price_float = float(total_price)
        order = Order.objects.create(user=request.user,address=address,total_price=total_price,payment_method=payment1)
    client = razorpay.Client(auth=(settings.KEY,settings.SECRET))
    payment = client.order.create({'amount':int(total_price_float*100) , 'currency':'INR','payment_capture':1})
...
def apply_coupon(request):
    ...
    if request.method == "POST":
        ...
        if Usercoupon.objects.filter(coupon=coup).exists():
            response_data = {
            'success': False,
...
def Removecoupon(request):
    cart=get_object_or_404(Cart,user=request.user)
    usercoupon=get_object_or_404(Usercoupon,coupon=cart.coupon)
    usercoupon.delete()
    cart.coupon = None
    cart.save()
    return redirect('cartdetail')
